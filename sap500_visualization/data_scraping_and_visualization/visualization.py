...
def create_bottom_figure(z_data, x_data, y_names, graph_type, metric_name, agg_name, data_processing="none",
    ...
    if graph_type == "Heatmap":
        ...
        fig.update_layout(title=f"UMAP Projection (neighbors={n_neighbors}, min_dist={umap_dist:.2f})", height=800,
                          ...
                          legend=dict(title="Sectors", orientation="v", yanchor="top", y=1, xanchor="left", x=1.02,
        ...
        vibrant_palette = (px.colors.qualitative.Plotly + px.colors.qualitative.Vivid[
            :-1] + px.colors.qualitative.Prism[:-1] + px.colors.qualitative.Set1[
                               :-2] + px.colors.qualitative.Set2 + px.colors.qualitative.Set3) * 10
        ...
        for i, name in enumerate(y_names):
            fig.add_trace(go.Scatter(
                x=x_data, y=z_data[i], name=name, mode="lines", line=dict(color=vibrant_palette[i]),
...
def main():
    ...
    app.layout = html.Div([
        ...
        html.Div([
            html.Div([
                ...
                dcc.Dropdown(id="ticker_selector", options=initial_stock_options, value=names[0] if names else None,
            ...
            html.Div([
                ...
                dcc.Dropdown(id="graph_type_dropdown", options=[{"label": " Heatmap", "value": "Heatmap"},
                                                                ...
                                                                {"label": " t-SNE Clusters",
                                                                 "value": "t-SNE Clusters"}], value="Heatmap",
                             clearable=False, persistence=True, persistence_type='local',
                             style={"marginBottom": "1rem"}),

                html.P("Sort Stocks By:", style={"fontWeight": "bold", "marginBottom": "0.5rem"}),
                dcc.Dropdown(id="sort_by_dropdown", options=[{"label": "Default (Original Order)", "value": "Default"},
                                                             {"label": "Alphabetical (A-Z)",
                                                              "value": "Alphabetical (A-Z)"},
                                                             {"label": "Alphabetical (Z-A)",
                                                              "value": "Alphabetical (Z-A)"},
                                                             {"label": "Correlation (Group Similar)",
                                                              "value": "Correlation"}], value="Default",
                             clearable=False, persistence=True, persistence_type='local',
                             style={"marginBottom": "1rem"}),
                ...
                dcc.Dropdown(id="corr_reference_stock",
                             options=[{"label": "None (Disabled)", "value": "NONE"}] + initial_stock_options,
                             value="NONE", clearable=False, persistence=True, persistence_type='local',
                             style={"marginBottom": "0.5rem"}),

                # Container that hides when disabled
                html.Div([
                ...
                html.P("Filter by Category (Sector):", style={"fontWeight": "bold", "marginBottom": "0.5rem"}),
                dcc.Dropdown(id="sector_selector",
                             options=[{"label": "All Categories", "value": "ALL"}] + [{"label": s, "value": s} for s in
                                                                                      sorted_sectors], value="ALL",
                             clearable=False, persistence=True, persistence_type='local',
                             style={"marginBottom": "1.5rem"}),

                html.P("Filter Specific Stocks:", style={"fontWeight": "bold", "marginBottom": "0.5rem"}),
                dcc.Dropdown(id="timeseries_selector", options=initial_stock_options, value=names.copy(), multi=True,
                             placeholder="Select stocks...", persistence=True, persistence_type='local',
                             style={"marginBottom": "0.5rem"}),
                html.Div([
    ...
    # Dynamic toggle & Live Preview callback for correlation filter
    @app.callback(
    ...
    def update_corr_ui_and_preview(corr_ref_stock, inclusion_range, selected_timeseries, selected_metric,
        ...
        if not selected_timeseries:
            return {"display": "block"}, "No stocks selected in filter."

        # Quick calculation of correlations for preview text
        selected_time_window_sec = (selected_time_window or 60) * MINUTE_SEC
        new_z, _ = aggregate_data(all_data, metric=selected_metric,
        ...
        return {"display": "block"}, f"Preview: Keeping {match_count} of {len(selected_timeseries)} stocks."
    ...
    @app.callback(
        ...
        Input("sector_selector", "value"), Input("select_all_btn", "n_clicks"), Input("unselect_all_btn", "n_clicks"),
    ...
    def update_price_graph_from_dropdown(selected_name):
        ...
        data = all_data[idx]
        return create_price_graph(data["Time"].values, data["Price"].values, selected_name)
    ...
    def update_bottom_graph(btn_clicks, selected_graph_type, data_processing, selected_timeseries, sort_by,
        ...
        if selected_timeseries:
            ...
            # --- INTUITIVE FILTER LOGIC (INCLUDE MIDDLE RANGE) ---
        ...
        bottom_fig = create_bottom_figure(filtered_z, new_x, filtered_names, selected_graph_type, selected_metric,
                                          selected_aggregation, data_processing, tsne_perp, umap_neigh, umap_dist)
